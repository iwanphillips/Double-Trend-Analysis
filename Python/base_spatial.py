...
def av_signal(T_, coor, step, per=[], season=None, weight=True, num=False, hem=None, ant2='west', ant=False):
    ...
    # to work on
    if type(coor)==xr.core.dataset.Dataset or type(coor)==xr.core.dataarray.DataArray:
        mask = coor
        mask = mask.rename({'lon': 'longitude','lat': 'latitude'})

        #lon = np.arange(-180, 180, step); lat = np.arange(-90, 90, step) #this works for east antarctic
        if ant2=='west':
            lon = np.arange(0, 360, step); lat = np.arange(-90, 90, step)     #this works for west antarctic
        else:
        ...
        #lat1, lat2, lon1, lon2 = id_lat[0], id_lat[-1], id_lon[0], id_lon[-1]
        #print("lat = ", lat1, lat2, lon1, lon2)
        mask = mask.sel(latitude=slice(lat2, lat1), longitude=slice(lon1, lon2)) # To try to speed up code/ stop crashing
    ...
    if (type(T_)==xr.core.dataset.Dataset or type(T_)==xr.core.dataarray.DataArray) and not lats: 
        lats = np.arange(lat2, lat1+0.01,step) # check order is correct
    #print("lats = ", lats)
    #print("lons = ", lons)
    ...
    for i in range(len(lons)): 
        for j in range(len(lats)): 
            '''I'm currently getting back lots of empty
               lists. Check that [i,j] actually overlaps with
               the mask or if there is some shift that prevents
               that.'''
        
            #if np.isnan(np.array(mask)[i,j]): # maybe [j, i]
            ...
            if np.isnan(Tf).any(): continue
            if not (Tf.tolist()):  
                #print("not Tf to list: ", Tf.tolist())
                continue
...
def eof_run(T_, npcs=15, coor=None, t_slice=[], w=True, season=None, sv=None, step=1, ant=False, ant2='west'): 
    ...
    if type(coor)==xr.core.dataset.Dataset or type(coor)==xr.core.dataarray.DataArray:
        ...
        mask = mask.rename({'lon': 'longitude','lat': 'latitude'})
        # If this line is producing an error then mask is probably incorrectly defined.
        ...
        #lat1, lat2, lon1, lon2 = id_lat[-1], id_lat[0], id_lon[-1], id_lon[0]
        #lat1, lat2, lon1, lon2 = id_lat[0], id_lat[-1], id_lon[0], id_lon[-1]
        #print("lat1: ", lat1, ", lat2: ", lat2, ", lon1: ", lon1, ", lon2: ", lon2)

        if ant==False:
            T = T.sel(latitude=slice(lat1, lat2), longitude=slice(lon1, lon2))
            #T = T.sel(latitude=slice(lat1, lat2))
            #T = T.sel(latitude=slice(lat1, lat2), longitude=slice(180 - lon1, 180 - lon2))
            #T = T.sel(latitude=slice(lat1, lat2), longitude=slice(180 - lon2, 180 - lon1))
            #T = T.sel(latitude=slice(lat1, lat2), longitude=slice(lon1+360, lon2+360))
            # maybe I need to fix the longitude by adding 180
            #T = T.sel(latitude=slice(lat1, lat2), longitude=slice(lon2, lon1))
            #T = T.sel(latitude=slice(lat1, lat2))
            T = T.where(mask.notnull())
        elif ant=='comp':
            print("complicated case: include all longitude")
            T = T.sel(latitude=slice(lat1, lat2))
            T = T.where(mask.notnull())

        else:
            # go over this to fix antarctica issue 
            print("complicated case due to longitudinal sign change")
            T1 = T.sel(latitude=slice(lat1, lat2), longitude=slice(-180,lon2))
            T2 = T.sel(latitude=slice(lat1, lat2), longitude=slice(150, 180))
            T = T1.combine_first(T2)

            T = T.where(mask.notnull()) # I think this should be here as well? 
...
def eof_plot(T_, nam, coor, a=1, b=1.01, sh='23', an=True, per=[], pcn=0, 
             proj='cartesian', cen=-90, shift=False, season=None, st='50', 
             step=1, w=True, ant=False, ant2='west'):
   
    weights_array, lons, lats, pc, eof, varfrac, lat1, lat2, lon1, lon2 = eof_run(T_, coor=coor, t_slice=per, season=season, 
                                                                                  step=step, w=w, ant=ant, ant2=ant2)
    #lat1, lat2, lon1, lon2 = int(coor[0]), int(coor[2]), int(coor[1]), int(coor[3])
    ...
    if proj=='polar': 
        #proj_ = ccrs.NorthPolarStereo() # ccrs.AzimuthalEquidistant(central_latitude=cen) # ccrs.NorthPolarStereo()
        proj_ = ccrs.AzimuthalEquidistant(central_latitude=cen)
    if proj=='cartesian': 
...
def eof_pcs(f, nam, coor, pcs=[1,2,3], npcs=40, per=[], weight=True, season=None, step=1, ant=False, ant2='west'):
    
    _, _, _, pc, eof, varfrac, _, _, _, _ = eof_run(f, npcs=npcs, coor=coor, t_slice=per, w=weight, season=season, step=step, ant=ant, ant2=ant2)
...
def recons(f, nam, coor, n=3, proj='cartesian', cen=-90, per=[], season=None, ant=False, clev=None, ant2='west'):
    #T = xr.open_dataset(f)
   
    weights_array, lons, lats, pc, eof, _, _, _, _, _ = eof_run(f, coor=coor, t_slice=per, season=season, ant=ant, ant2=ant2)
    ...
    f.savefig('ecmwf/recon_eof_n' + str(n) + '.png', format='png', dpi=120, bbox_inches="tight")


    # adding pc's code

def add_pc_plot(pcs, av_sig, par='m', name=None, sv=None, s=10): 
    ...
    sns.set_style("white", rc=params) # , rc=params
    a_ = 0.5
    fig = plt.figure(figsize=(a_*5, a_*3)) 
    bax = brokenaxes( 
...
def add_pc_plot_ax(ax, pcs, av_sig, par='m', name=None, 
                   xax=True, n_pcs=None, n=15): 
                                                                                                            
    par_l = np.array(pcs[par])
    if n_pcs: par_l = par_l[:n_pcs]
    y = np.append(par_l, av_sig[par][0])      
    x = np.append(list(range(1,len(y))), 1000) 
    ...
    bax.axs[1].set_xticks([1000])
    bax.axs[1].set_xticklabels([r'Av.']) # r'$\infty$'

    bax.grid(axis='both', which='major', ls='-', alpha=0.3)
    bax.grid(axis='both', which='minor', ls='--', alpha=0.3)

    if xax==True:
        bax.axs[0].set_xlabel('no PCs', labelpad=n)
        bax.axs[0].xaxis.set_label_coords(0.65, -0.16)
    if name != None: bax.axs[0].set_ylabel(name, labelpad=n)
...
def pc_seas(f, nam, coor, pcl, per, seq='eig', season=None, 
            step=1, df=pd.DataFrame({}), npcs=51, ant2='west', ant=False, ant2_av=False):

    #if season: print(season)

    eof_, eig_, pc_ = eof_pcs(f, nam, coor, pcs=list(range(1,npcs)), npcs=npcs, 
                                 weight=True, per=per, season=season, step=step, ant2=ant2, ant=ant) 
    try:
        if ant2_av==False: ant2_av = ant2
        _, l_a = av_signal(f, coor, step, weight=True, per=per, season=season, ant2=ant2_av, ant=ant) 
    except:
...
def print_pc_seas(f, nam, coor, seq='eig', step=1, 
                  npcs=51, per=['1979-01-01', '2022-12-31'], ant2='west', ant=False, ant2_av=False): 
    pcl = ['1', '2', '3', '4', '12', '34', 'av']

    df = pc_seas(f, nam, coor, pcl, per, seq=seq, season=None, step=step, npcs=npcs, ant2=ant2, ant=ant, ant2_av=ant2_av)
    df = pc_seas(f, nam, coor, pcl, per, seq=seq, season='spring', step=step, df=df, npcs=npcs, ant2=ant2, ant=ant, ant2_av=ant2_av)
    df = pc_seas(f, nam, coor, pcl, per, seq=seq, season='summer', step=step, df=df, npcs=npcs, ant2=ant2, ant=ant, ant2_av=ant2_av)
    df = pc_seas(f, nam, coor, pcl, per, seq=seq, season='autumn', step=step, df=df, npcs=npcs, ant2=ant2, ant=ant, ant2_av=ant2_av)
    df = pc_seas(f, nam, coor, pcl, per, seq=seq, season='winter', step=step, df=df, npcs=npcs, ant2=ant2, ant=ant, ant2_av=ant2_av)
...
def p_bar_plot(df, s=21, sv=None, lloc='upper right'):
    ...
    a_ = 0.5
    fig, ax = plt.subplots(figsize=(a_*6, a_*5))
